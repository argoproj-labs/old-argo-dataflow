...
type Impl struct {
	key        string
	endOfGroup string
	format     dfv1.GroupFormat
	prog       *vm.Program
	endProg    *vm.Program
}

func New(key string, endOfGroup string, format dfv1.GroupFormat) *Impl {
	return &Impl{key: key, endOfGroup: endOfGroup, format: format}
}
...
func (i *Impl) Init(ctx context.Context) error {
	...
	prog, err := expr.Compile(i.key)
	if err != nil {
		return fmt.Errorf("failed to compile %q: %w", i.key, err)
	}
	i.prog = prog
	endProg, err := expr.Compile(i.endOfGroup)
	if err != nil {
		return fmt.Errorf("failed to compile %q: %w", i.endOfGroup, err)
	}
	i.endProg = endProg
	return nil
}
...
func (i *Impl) Exec(ctx context.Context, msg []byte) ([]byte, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("failed to run program %q: %w", i.key, err)
	}
	group, ok := res.(string)
	if !ok {
	...
	return withLock(dir, func() ([]byte, error) {
		...
		res, err = expr.Run(i.endProg, env)
		if err != nil {
			return nil, fmt.Errorf("failed to run program %q: %w", i.endOfGroup, err)
		}
		end, ok := res.(bool)
		if !ok {
			return nil, fmt.Errorf("end-of-group expression %q must return a bool", i.endOfGroup)
		}
		if !end {
			return nil, nil
		}
		...
		if err != nil {
			return nil, fmt.Errorf("failed to read dir %q: %w", i.endOfGroup, err)
		}
		...
		return nil, fmt.Errorf("unknown group format %q", i.format)
	})
}
