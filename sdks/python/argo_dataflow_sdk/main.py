...
AUTH_FILE_PATH = environ.get('AUTH_FILE', '/var/run/argo-dataflow/authorization')
...
class ProcessHandler:
          ...
          for val in self.handler:
            async with session.post(GENERATOR_STEP_SINK, data=val) as response:
              if response.status > 299:
                ...
                logging.warn('Message could not be processed', response.status, response_body)
  ...
  def start(self, handler):
    ...
    self.app.add_routes([web.get('/ready', self.__ready_handler), web.post('/messages', self.__messages_handler)])
    self.app.on_shutdown.append(self.__on_shutdown)
    logging.debug(f"Server starting at http://{HOST_NAME}:{SERVER_PORT} with pid {getpid()}")
    
    web.run_app(self.app, host=HOST_NAME, port=SERVER_PORT, print=logging.debug)
  ...
  async def __start_generator(self, handler):
    ...
    self.app = web.Application()
    self.app.add_routes([web.get('/ready', self.__ready_handler), web.post('/messages', self.__empty_messages_handler)])
    self.app.on_startup.append(self.__start_background_tasks)
    self.app.on_cleanup.append(self.__cleanup_background_tasks)
    self.app.on_shutdown.append(self.__on_shutdown)
    logging.debug(f"Generator Server starting at http://{HOST_NAME}:{SERVER_PORT} with pid {getpid()}")
    runner = web.AppRunner(self.app)
    await runner.setup()
    site = web.TCPSite(runner, HOST_NAME, SERVER_PORT)
    await site.start()
    logging.info(f"======== Running on {site.name} ========\n(Press CTRL+C to quit)")
